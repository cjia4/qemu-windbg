...
int x86_cpu_gdb_read_register(CPUState *cs, GByteArray *mem_buf, int n)
{
    ...
    if (n < CPU_NB_REGS) {
        ...
        case IDX_CS_BASE:
            return gdb_read_reg_cs64(env->hflags, mem_buf, env->segs[R_CS].base);
        case IDX_SS_BASE:
            return gdb_read_reg_cs64(env->hflags, mem_buf, env->segs[R_SS].base);
        case IDX_TR_BASE:
            return gdb_read_reg_cs64(env->hflags, mem_buf, env->tr.base);
        }
    }
    return 0;
}
...
int x86_cpu_gdb_write_register(CPUState *cs, uint8_t *mem_buf, int n)
{
    ...
    if (n < CPU_NB_REGS) {
        ...
        case IDX_CS_BASE:
            len = gdb_write_reg_cs64(env->hflags, mem_buf, &tmp);
            env->segs[R_CS].base = tmp;
            return len;
        case IDX_SS_BASE:
            len = gdb_write_reg_cs64(env->hflags, mem_buf, &tmp);
            env->segs[R_SS].base = tmp;
            return len;
        case IDX_TR_BASE:
            len = gdb_write_reg_cs64(env->hflags, mem_buf, &tmp);
            env->tr.base = tmp;
